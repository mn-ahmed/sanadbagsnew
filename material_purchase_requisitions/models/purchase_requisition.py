...
class MaterialPurchaseRequisition(models.Model):
    ...
    def requisition_confirm(self):
        for rec in self:
            ...
            if rec.env.user.has_group('material_purchase_requisitions.group_create_requisition') and not rec.env.user.has_group('material_purchase_requisitions.group_purchase_requisition_department'):
...
# vim:expandtab:smartindent:tabstop=4:softtabstop=4:shiftwidth=4:
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        result = super(MaterialPurchaseRequisition, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
            submenu=submenu)
...
#                 rec.dept_manager = False
        ...
        for rec in self:
            if rec.state == 'dept_confirm':
                ...
                if rec.env.user.has_group('material_purchase_requisitions.group_purchase_requisition_manager') or rec.env.user.has_group('material_purchase_requisitions.group_store_keeper') :
                    rec.dept_manager = True      
