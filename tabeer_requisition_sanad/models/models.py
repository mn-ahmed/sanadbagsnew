...
class MaterialRequisitioninh(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        result = super(MaterialRequisitioninh, self).fields_view_get(
            ...
            submenu=submenu)
#         if self.env.user.has_group('material_purchase_requisitions.group_purchase_requisition_user') and self.env.user.has_group('tabeer_requisition_sanad.group_requisition_user'):
...
#             temp.set('edit', '0')
#             result['arch'] = etree.tostring(temp)
        if not self.env.user.has_group('tabeer_requisition_sanad.group_requisition_user'):
            temp = etree.fromstring(result['arch'])
            temp.set('create', '0')
            temp.set('edit', '0')
            result['arch'] = etree.tostring(temp)
    ...
    def action_show_po(self):
        for rec in self:
            tree_view_id = self.env.ref('tabeer_requisition_sanad.purchase_order_tree_req').ids
            form_view_id = self.env.ref('tabeer_requisition_sanad.purchase_order_form_req').ids
            return {
    ...
    def show_picking(self):
        for rec in self:
            tree_view_id = self.env.ref('tabeer_requisition_sanad.vpicktree_req').ids
            form_view_id = self.env.ref('tabeer_requisition_sanad.view_picking_form_req').ids
            return {
                'type': 'ir.actions.act_window',
                'name': 'Purchase',
                'views': [[tree_view_id, 'tree'], [form_view_id, 'form']],
